...
class IntegerRange
  ...
  def mergeable?(other)
    if (@top + 1) == other.bottom
      ...
      return true
    elsif (@bottom <= other.bottom) && (other.bottom <= @top)
      return true
    elsif (other.bottom <= @bottom) && (@bottom <= other.top)
      return true
    end
  ...
  def merge(other)
    fail ArgumentError, 'The ranges cannot be merged' unless mergeable?(other)
